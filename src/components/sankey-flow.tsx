...
function layoutNodes(scene: DisplayScene, width: number, height: number): PositionedNode[] {
  if (!scene.centerId || scene.nodes.length === 0) return [];
  ...
  for (const zone of ZONES) {
    const zoneNodes = scene.nodes.filter((node) => node.zone === zone);
  ...
  const simulationNodes: SimulationLayoutNode[] = scene.nodes.map((node) => {
...
export function SankeyFlow({
  ...
  const positionedNodes = useMemo(
    () => layoutNodes(scene, viewportWidth, viewportHeight),
